...
|---|---|---|---|---|---|---:|---:|---|
| 39 | 2026-08-12 | **pre-registered, SI 30-min 2009-2016** | micro silver (SIL) | passive execution on SI/ASIA/q4: TAKE vs PASSIVE-ENTRY vs PASSIVE-BOTH x m=0,1,3 ticks | signal UNCHANGED from run 34, per-signal stats with mandatory unfilled counterfactuals, Bonferroni 0.0071 | 3,109 signals | best +2.72/signal | **ALL SEVEN FAIL. THE STOPPING RULE IN THE PRE-REGISTRATION IS TRIGGERED AND THE SEARCH IS OVER.** Attacked cost instead of hunting a 97th signal, because silver was the only structure whose 4.3x gap execution could plausibly close. **The counterfactual column is the whole result: at EVERY m the unfilled signals are worth several times the filled ones** -- m=0 fill 84%, filled +$1.32 vs unfilled CF **+$38.43** (29x); m=1 73%, +$3.01 vs +$32.68 (11x); m=3 52%, +$5.27 vs +$25.44 (4.8x). Mechanism is exact: the trade is a fade, so your offer fills ONLY when price keeps going against you, and when price reverses immediately -- the winning case -- it never reaches your limit. You do not choose the trades; the market hands you the failures. PASSIVE-BOTH/m3 beat its cost (+$1.73 vs $1.20 RT) and is still worthless: p 0.190, and foregone trades worth 7.6x realised ones. TAKE reproduced run 34 (+7.29 vs +7.22), confirming the signal was undisturbed. **FIRST RUN WAS VOID**: limit rested on the wrong side (a short offering BELOW), caught by the pre-registered fill-rate check when rates came back 84/95/99% RISING with m. Third defect that check has caught. Prediction 4 of 6: right on outcome and mechanism, wrong that larger m would select harder for continuation -- it selects for a LARGER overshoot and so BETTER filled trades, but the missed trades stay worth more at every distance. |
| 38 | 2026-08-12 | **pre-registered, ES 5-min 2009-2016** | ES as MES | volume-conditioned fade, 3 terciles x q=1,2,4,8 | volume ranked WITHIN time-of-day causally, non-overlapping, DELAY primary, Bonferroni 0.0042 | 726,038 | best +0.26/trade | **ALL 12 FAIL AS STRATEGIES. TWO PASS AS MEASUREMENTS. The registered contrast landed inside its stated range and is the deliverable.** THIN median keep under 1-bar delay **20%** (predicted <30%), HEAVY **86%** (predicted 60-100%). **THIN/q1 has t = 22.08 on 91,962 trades and keeps -3%** -- Roll's model in one line, bounce lives entirely in the adjacent print. **HEAVY/q1 and q2 keep 57% and 67% with DELAYED t of 3.68 and 3.35**, which bounce cannot produce and which the diagnostic's lag-2 term predicted. Only the **third delay-robust signal in 96 cells** (after SI/ASIA/q4): HEAVY/q2 is +$0.097 delayed against $3.70, **2.6% of cost**; HEAVY/q1 is 1.9%. Real, and off by a factor of forty. **THE EDGE DOES NOT SCALE WITH HOLDING TIME**: 0.120, 0.143, 0.067, 0.123 across q=1,2,4,8 -- flat, not sqrt(q). A drift scales because it is a return per unit time; a fixed dollar amount that does not scale is a ONE-OFF CORRECTION, a transient liquidity event absorbed once. That closes the hold-longer escape route from run 33. Prediction 6 of 7; the miss is the sqrt(q) scaling, the SAME miss as run 33, which now looks like a wrong model rather than a bad estimate. |
| 37 | 2026-08-12 | diagnostic, not a strategy | ES 5-min | return structure conditioned on relative volume + BVC order-flow imbalance | volume normalised within time-of-day, causally | 554,501 bars | n/a | **FIRST DIAGNOSTIC IN THE PROJECT TO USE VOLUME FOR ANYTHING BUT THE ROLL.** Lag-1 gradient -0.0751 THIN / -0.0438 MID / -0.0211 HEAVY vs 2se 0.0047: this is run 33's bounce hypothesis LANDING, not a discovery -- bounce is a fixed tick amount so as a share of variance it is largest where the bar is quietest, and thin bars carry 3.6x the reversion of heavy ones. **The new term is at lag 2**: THIN is lag-1 only (+0.0011, dead on zero, pure Roll signature) while HEAVY carries **-0.0250**, ten se out and MORE negative than its own lag 1. Roll predicts nothing at lag 2, so that component is not bounce. **BVC-OFI is dead**: forward IC -0.0161 next-bar, -0.0057 delayed, vs a 2se band of 0.0027, under a cent per trade. Recorded precisely: BVC-OFI is a volume-weighted transformation of the SAME bar's price change, NOT a substitute for true signed order flow, so this closes the proxy and not the input class. Volume conditioning separated two things previously mixed in one number. |
| 36 | 2026-08-12 | **pre-registered, confirmatory, 30-min 2009-2016** | ES+NQ as MES+MNQ | ES/NQ spread fade, 3 windows x q=1,2,4 | 1 MES vs 1.32 MNQ causal dollar-neutral, non-overlapping, DELAY criterion primary, Bonferroni 0.0056 | 125,855 | best +0.43/trade | **ALL NINE FAIL. THE RELATIVE-VALUE FAMILY IS CLOSED, MECHANISTICALLY.** **Edge size and delay-survival are almost perfectly anticorrelated**, which is the whole result: ASIA/q1 has **t = 25.09, the largest t-statistic in the project**, on 15,945 trades, and stepping over ONE 30-minute bar takes it to t 1.14, keeping **6%**. ALL/q1 t 12.10 keeps -6%. ASIA/q2 t 11.96 keeps 16%. The only cells that survive delay (ALL/q4 74%, NY/q1 106%) are the two with no edge to survive with. **There is no cell where a real edge and delay-robustness coexist.** Both pre-registered mechanisms are visible: differencing makes each leg's bounce ADD on a smaller base (spread sigma 7.11bp vs 15.63 ES, 17.13 NQ), and NQ trades 229 contracts per 30-min bar in Asia vs ES's 2,125 so the legs are priced minutes apart. Asia's raw edge is 6x NY's on a fraction of the volume and it is Asia that collapses. Even undelayed, the best cell is 6.5% of the $6.60 pair round turn. Prediction 5 of 6 correct; the miss is that I said the delayed Asia number would be negative and it is +$0.025, statistically zero. Overstated a sign I had no basis for. |
| 35 | 2026-08-12 | diagnostic, not a strategy | ES/NQ log spread | autocorrelation by lag, 94,826 aligned bars | returns within (session, contract-pair), never spanning a roll or halt | 92,749 bars | n/a | **CLOSED THE RELATIVE-VALUE FAMILY BEFORE A SINGLE TRIAL WAS SPENT.** The spread looks spectacular: ac(1) **-0.2116** in Asia (VR(2) 0.790 at z **-18.8**, the largest anywhere in this project), -0.0774 London, -0.0516 overall. **But it is ALL at lag 1.** Lags 2-5 are inside or barely outside the Bartlett band in every window. Roll's bounce model predicts lag-1 and nothing else; genuine multi-bar reversion would show at lags 2-4 and does not. The tell is the scaling: the signal is 10x stronger in Asia than NY while Asian NQ volume is 56x lower (229 vs 12,918 contracts/bar). It tracks illiquidity, not economics. Diagnostic ordering applied correctly this time -- measure the property, THEN pre-register a test of it. |
| 34 | 2026-08-11 | **pre-registered, 30-min, 2009-2016** | ES / GC / SI as micros | the 4 cells surviving bounce dilution: ES-LONDON, SI-ASIA, SI-LONDON fade; GC-NY, SI-NY follow | non-overlapping, no stop, no filter, 1-bar-delay as a headline criterion, Bonferroni 0.0025 | 78,000+ | best +7.22/trade | **NO STRATEGY PASS. ONE MEASUREMENT PASS -- the best result in the project, and it still does not pay.** At 30-min bars ES ALL is a perfect random walk (ac1 exactly 0.0000, VR(2) 0.999 z -0.1), confirming run 33: the 5-min z of -7.8 was the spread, and diluting the bar 6x dilutes the artifact away. **SI/ASIA/q4** (fade the prior 2-hour move in Asian-session silver, hold 2 hours): **+$7.22/trade, t 3.09, p 0.0010, passes Bonferroni, 7 of 7 years positive, and KEEPS 112% of its edge under one-bar delayed execution** -- an edge that grows under delay cannot be bounce. **It fails on cost by 4-8x**: micro silver's round turn is $31.20 and the edge is 23% of it. 55% of P&L is 2011 (the $18 to $49 spike); ex-2011 it is +$3.87 on 2,493 trades with 6 of 6 years positive, so the effect is persistent but half the headline. Robust to the cost convention: the project models 2 full spreads per round turn, and even at the looser 1-spread convention ($16.20) it is 4x short. ES/LONDON passes the delay test emphatically (145% and 184%, edge LARGER when delayed, consistent with its ac1 being 10x what Roll predicts) but reaches only +$0.23 to +$0.58 vs $3.70. **GC/NY and SI/NY momentum evaporated as predicted** (t 0.82 and 0.96) -- the only VR>1 cells in an 80-cell table where ~4 spurious |z|>2 are expected. Live SI/SIL spread snapshots returned empty, so the full-size spread is unmeasured and is NOT estimated. **First of ten predictions where the mechanism was right, not just the verdict.** |
| 33 | 2026-08-11 | **pre-registered, ES 2009-2016, 5-min** | ES as MES | reversion edge vs holding horizon, q = 1..78 bars | fade the prior q-bar move, hold q, non-overlapping, no stop, no filter, Bonferroni 0.00625 | 492,564 | best +1.51/trade | **ALL EIGHT FAIL, AND THE FAMILY IS CLOSED WITH A MECHANISM.** The edge does NOT scale as sqrt(q) as registered; it **decays from the first bar**: +$0.205 (t 19.4) at 5 min, +$0.161, +$0.181, then $0.049 at 30 min, $0.019 at 60, **negative** at 120 and 240. **The short-horizon reversion is BID-ASK BOUNCE.** Two independent confirmations. (1) Roll's estimator on the lag-1 autocovariance (-0.02634 pt^2) recovers an effective spread of **0.3246 pt = 1.30 ticks**, exactly right for ES, worth **$1.62** -- the $0.205 "edge" is eight times smaller than the spread that manufactures it. (2) Delaying execution one bar destroys 76% of it and two bars destroys 95%: t falls 19.36 -> 3.61 -> 0.72. A real inefficiency does not evaporate in five minutes; bounce lives entirely in the adjacent print. q=78's +$1.51 at t 2.24 fails p and is the most concentrated cell in the table (t-5% -4.07); out-of-pattern after seven smoothly decaying horizons, it is noise. **This closes ES-only price-only directional intraday strategies at 5-min to 4-hour horizons**, the family containing all 43 prior cells: they failed because the thing they were timing is not there. **MY OWN CRITERION FAILED HERE**: t-5% is meaningless at n=220,000, where deleting 11,000 fat-tail observations from a near-zero-mean distribution is negative by construction. Recorded rather than leaned on. |
| 32 | 2026-08-11 | diagnostic, not a strategy | ES 5-min | return-generating process: autocorrelation + Lo-MacKinlay variance ratios | returns strictly within contract and session, never spanning a roll or the halt | 550,050 bars | n/a | **43 strategy cells had been tested without anyone measuring the property they all depend on.** Lag-1 autocorrelation negative in every window and far outside the Bartlett band: -0.031 ALL, -0.055 ASIA, -0.040 LONDON, -0.021 NY vs a 2se band of 0.003-0.005. **Every variance ratio below 1**, most decisively: VR(2) 0.969 z **-7.8**, VR(4) 0.939 z -8.1, VR(32) 0.926 z -3.5. Below 1 = mean-reverting, so the random walk is rejected **in the direction opposite to what every breakout cell bet on**. Explains A2L's significantly negative gross, breaks reverting, STRONG removing losers, and retracement entries selecting failures, all as one property. Motivated run 33, which showed the property is the spread. **This measurement should have been cell 1, not cell 32.** |
